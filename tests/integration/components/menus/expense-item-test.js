...
module('Integration | Component | menus/expense-item', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.dom('nav ul li:nth-of-type(2) a').containsText('Attachments');
    assert.dom('nav ul li:nth-of-type(2) a svg').exists();
    assert.dom('nav ul li:nth-of-type(2) a svg').hasClass('fa-paperclip');
  ...
  test('it renders expense attachments button', async function(assert) {
    const router = this.owner.lookup('service:router');
    router.currentRouteName = '';
  ...
  test('it should not render expense attachments button', async function(assert) {
    const router = this.owner.lookup('service:router');
    router.currentRouteName = 'expenses.attachments';

    await render(hbs`<Menus::ExpenseItem />`);

    assert.dom('nav ul li:nth-of-type(2) a').doesNotExist();
    assert.dom('nav ul li:nth-of-type(2) span').exists();
    assert.dom('nav ul li:nth-of-type(2) span').containsText('Attachments');
  });
